...
gridsims <- subset(gridsim,delta>0 & n1 > 1)
...
ggplot(gridsims,aes(delta,-relbias))+geom_line(subset=.(sigma==.5 & n1 <= 20),aes(colour=factor(n1)))+scale_color_grey()+geom_text(aes(x=idx,y=-relbias[x==,label=as.character(n1)),subset=.(sigma==.5,delta==1)) #stat_function(fun=function(x) {.03/pmax(.5,c(x))^3},colour='red')


max.bias <- ddply(gridsims,"n1",summarise,
...
cplot <- ggplot(dfa)+
    ...
    facet_grid(variable~method,scale='free_y',labeller=lverbose(string='Actual - Nominal Coverage [%]')) +xlab(expression(paste("Total First Stage Sample Size ",2*n[1])))+ylab('Value')+theme_bw()+xlim(c(2,150))
print(cplot)
...
<<eval=F>>=
cairo_ps('maxbias.eps',height=5,width=5)
print(mplot)
dev.off()
