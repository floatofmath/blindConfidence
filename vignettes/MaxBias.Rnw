...
gridsim <- within(gridsim,relbias <- mean.bias/sqrt(vm),uc.relbias <- uc.mean.bias/sqrt(uc.vm))
...
qplot(2*n1,value,data=df,geom='line')+facet_grid(variable~scale,scale='free_y')+xlab('Total first stage sample size')+ylab('Value')
