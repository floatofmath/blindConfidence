...
mplot <- qplot(2*n1,value,data=df[df$scale=="absolute",],geom='line')+facet_grid(variable~method,scale='free_y',labeller=lverbose)+xlab('Total first stage sample size')+ylab('Value')+theme_bw()
print(mplot)
