...
mplot <- ggplot(dfa,aes(2*n1,value))+
    ...
    geom_smooth(subset=.(variable!="maximum mean bias")) +
    facet_grid(variable~method,scale='free_y',labeller=lverbose) +xlab('Total first stage sample size')+ylab('Value')+theme_bw()+xlim(c(2,150))
print(mplot)
