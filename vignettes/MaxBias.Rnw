...
<<>>=
G <- expand.grid(delta=round(seq(0,4,.2),2),
                 sigma=round(seq(.2,2,.2),2),
                 d = c(1),
                 s = round(seq(.5,2,.2),2))
...
\subsection{Maximum bias for a given first stage sample size}
...
ag.bias <- aggregate(cbind(-mean.bias,-variance.bias,upper.prob-0.025,lower.prob-0.025,total.prob-0.05)~n1,FUN=max,data=gsim)
uc.ag.bias <- aggregate(cbind(-uc.mean.bias,-uc.variance.bias,uc.upper.prob-0.025,uc.lower.prob-0.025,uc.total.prob-0.05)~n1,FUN=max,data=gsim)

names(ag.bias) <- c('n1','mean.bias','var.bias','upper','lower','twosided')
names(uc.ag.bias) <- c('n1','mean.bias','var.bias','upper','lower','twosided')
par(mfrow=c(1,2))
plot(mean.bias~I(n1*2),ag.bias,type='b',ylim=c(0,.2))
abline(h=0.005)
plot(mean.bias~I(n1*2),uc.ag.bias,ylim=c(0,.2),type='b')
@ 
