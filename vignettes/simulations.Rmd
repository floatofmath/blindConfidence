...
considerable amount of main memory - publication results were computed
on a simulation server providing 40 processor cores and 64 GB of shared
memory. Note that the code has been optimized to run on Linux, so while
the package should work fine on Windows in theory, it has not been
tested.
...
lverbose = function(variable,value){
    if(variable == "s"){
        sapply(value,function(val) substitute(paste(sigma[0]," = ",foo,", ",2*n[1]," = ",n1,sep=''),
                                              list(foo=val,n1=2*ceiling(1/2*zss(val,1,.025,.2)))))
    } else {
...
## simulations are run.
G <- expand.grid(delta=round(seq(-2,2,.1),1),
...
tupd.mean.bias <- ggplot(gridsim,mapping=aes(x=delta,y=mean.bias)) +
    ...
    facet_grid(s~sigma,labeller=lverbose,scales='free_y') + theme_bw() +
...
tupd.var.bias <- ggplot(gridsim,mapping=aes(x=delta,y=variance.bias)) +
    ...
    facet_grid(s~sigma,labeller=lverbose,scales='free_y') + theme_bw() + #ylim(-0.3,0.1) +
...
tupd.vm.bias <- ggplot(gridsim,mapping=aes(x=delta,y=vm)) +geom_line(lty=1) +
    ...
    facet_grid(s~sigma,labeller=lverbose,scales='free_y') + theme_bw() + geom_blank(aes(x=delta,y=ylow)) +
...
tupd.uc.vm.bias <- ggplot(gridsim,mapping=aes(x=delta,y=uc.vm)) +
    ...
    geom_path(aes(y=uc.exv),lty=3) + facet_grid(s~sigma,labeller=lverbose,scale='free_y') +
    theme_bw() + xlab(expression(delta)) + geom_blank(aes(x=delta,y=ylow)) +
...
## gridsim <- within(gridsim,{relbias <- mean.bias/(delta+sqrt(vm))
##                            uc.relbias <- uc.mean.bias/(delta+sqrt(vm))})
gridsim <- within(gridsim,{relbias <- mean.bias/(delta)
                           uc.relbias <- uc.mean.bias/(delta)})


ns <- sort(unique(gridsim$n1))
idx <- seq(.5,2,length.out=length(ns))
...
max.bias <- within(max.bias,{max.mean.bias <- max.mean.bias/max.mean.bias.delta
                             uc.max.mean.bias <- uc.max.mean.bias/uc.max.mean.bias.delta})
...
mplot <- ggplot(dfa)+
    geom_line(aes(2*n1,100*value),subset=.(variable=="maximum mean bias")) +
    geom_smooth(aes(2*n1,value),subset=.(variable!="maximum mean bias")) + geom_point(aes(2*n1,value),subset=.(variable!="maximum mean bias"),size=1) +
    facet_grid(variable~method,scale='free_y',labeller=lverbose) +xlab(expression(paste("Total First Stage Sample Size ",2*n[1])))+ylab('')+theme_bw()+xlim(c(2,150))
print(mplot)
...
max.coverage <- ddply(gridsim,"n1",summarise,
...
df <- melt(max.coverage,id='n1')
df <- within(df,scale <- c("absolute","relative")[grepl("rel",variable)+1])
df <- within(df,method <- factor((2:1)[grepl("uc",variable)+1],labels=c("unadjusted","adjusted")))
df <- within(df,variable <- sub("rel\\.","",variable))
df <- within(df,variable <- sub("uc\\.","",variable))
df <- within(df,variable <- factor(variable,labels=c('noncoverage',expression(delta),expression(sigma))))
dfa <- df[df$scale=="absolute",]
scalehelper <- data.frame(n1=rep(-10,3),variable=factor(1:3,label=c("noncoverage",expression(delta),expression(sigma))),value=rep(0,3),scale=rep("absolute",3),method=rep("adjusted",3))
dfa <- rbind(dfa,scalehelper)
cplot <- ggplot(dfa)+
    geom_line(aes(2*n1,100*value),subset=.(variable=="noncoverage")) +
    geom_smooth(aes(2*n1,value),subset=.(variable!="noncoverage")) + geom_point(aes(2*n1,value),subset=.(variable!="noncoverage"),size=1) +
    facet_grid(variable~method,scale='free_y',labeller=lverbose(string='Actual - Nominal [%]')) +xlab(expression(paste("Total First Stage Sample Size ",2*n[1])))+ylab('Value')+theme_bw()+xlim(c(2,150))
print(cplot)
...
runs=5*10^6
casesim <- t(simplify2array(mclapply(1:nrow(G),
                                     function(i) {c(G[i,],
                                                    simVBIA(delta=G[i,]$delta,
                                                            sigma=G[i,]$sigma,
                                                            d=G[i,]$d,s=G[i,]$s,
...
runs <- 10^5
bsim <- do.call(rbind,lapply(1:nrow(G),
                             function(i) {c(G[i,],
...
mplot <- ggplot(df) + facet_grid(variable~sigma,scales='free_y',labeller=lverbose) +
    ...
    geom_path(aes(delta,mean.upper.bias),lty=3,col='red',subset=.(variable  == "Bias Of The Mean"))+
    geom_path(aes(delta,-mean.upper.bias),lty=3,col='red',subset=.(variable  == "Bias Of The Mean"))+
    geom_path(aes(delta,brannath),lty=3,col='darkgreen',subset=.(variable  == "Bias Of The Mean" & brannath < 1.6))+
    geom_path(aes(delta,-brannath),lty=3,col='darkgreen',subset=.(variable  == "Bias Of The Mean" & brannath < 1.6))+
    ...
    geom_path(aes(x=delta,y=.025-uc.upper.prob),lty=2,subset=.(variable=="Actual - Nominal")) +
    geom_line(aes(x=delta,y=0),colour='gray',subset=.(variable=="Actual - Nominal")) +
    geom_path(aes(x=delta,y=.025-uc.lower.prob),lty=3,subset=.(variable=="Actual - Nominal")) +
        geom_path(aes(x=delta,y=.05-uc.total.prob),subset=.(variable=="Actual - Nominal")) +
    geom_blank(aes(x=-5,y=-0.02),subset=.(variable=="Actual - Nominal")) +
    geom_blank(aes(x=-5,y=0.01),subset=.(variable=="Actual - Nominal")) + ylab('')+xlab(expression(delta))+theme_bw()
