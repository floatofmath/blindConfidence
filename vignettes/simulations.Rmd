...
For each scenario we perform $10^6$ simulation runs.
 ...
 Initially $10^6$ simulations were performed for a range of true
  mean  differences $\delta\in[0,4]$ and standard deviations
  $\sigma\in[.5,4]$ in steps of $0.1$. In a second step the grid was
  refined in a vicinity of parameter values at which maxima were
  obtaind   which was subsequently refined
  in the vicinity of )
...
Finally we selected the maximal values of the bias and resimulate to
any selection bias.
...
3. Estimate the absolute variance using simulations setting $\delta=0$
   and $\sigma=50$ (i.e. approaching $\Infty$)
...
`r round(casesim[uc.max.var.bias,sigma],2)`$ (\ie it reaches the
