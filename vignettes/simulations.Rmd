...
vplot <- ggplot(dfa,aes(n1,value))+
    geom_line(subset=.(variable=="maximum variance bias")) + geom_line(aes(n1,theoretical),subset=.(variable=="maximum variance bias" & method=='unadjusted'),linetype='dotted') + 
...
options(mc.cores=min(20,detectCores()-1))
maxcoveragesim2 <- simulate_batch(G2coverage,10^7)
## reshape and resim
...
options(mc.cores=min(10,detectCores()-1))
maxcoveragesim3 <- simulate_batch(maxcoverageopt2,5*10^7)
