...
mplot <- ggplot(dfa)+
    ...
    geom_smooth(aes(n1,value),data=subset(dfa,variable!="str_mmb")) + geom_point(aes(n1,value),data=subset(dfa,variable!="str_mmb"),size=1) +
...
vplot <- ggplot(dfa,aes(n1,value))+
    ...
    geom_smooth(data=subset(dfa,variable==levels(variable)[3])) + geom_point(data=subset(dfa,variable!="str_mvb"),size=1) + geom_smooth(data=subset(dfa,variable==levels(variable)[2])) +
    facet_grid(variable~method,scale='free_y',labeller=lverbose) +xlab(expression(n[1]))+ylab('')+theme_bw()+xlim(c(2,50))
print(vplot)
...
close to zero. Consequently we rerun the simulation restricting
$\delta$ to a fixed value of zero. Also for the unadjusted
reassessment rule, values of $\sigma$ that maximise the absolute
variance bias are consistently found at the upper border of the
simulation grid. It appears that the maximum is obtained for large
values of $\sigma$. Consequently we ran a simulation setting $\delta$
to zero and $\sigma$ to 50. This increased the variance of the
...
results. The results of this simulation are shown in the following
figure. Note that for the unadjusted rule the estimated maximum
absolute bias is practically equal to the theoretical bound derived in
Theorem ??. 
...
options(mc.cores=min(20,detectCores()-1))
maxvariancesimR2 <- simulate_batch(G2varianceR,10^7,use=T)
## reshape and resim
...
plot_grid(G2varianceR,maxvarianceoptR1,maxvarianceoptR2,'delta')
...
max.varbias2 <- ddply(maxvariancesimRL3,"n1",transform,
                  max.variance.bias= variance.bias,
                  uc.max.variance.bias= uc.variance.bias,
                      method=c("variance.bias"="adjusted","uc.variance.bias"="unadjusted")[what.max],
                  theoretical = bound
...
vplot <- ggplot(dfa,aes(n1,value))+
    geom_line(aes(n1,-value),data=subset(dfa,variable=="str_mvb")) + geom_line(aes(n1,-theoretical),data=subset(dfa,variable=="str_mvb" & method=='unadjusted'),linetype='dotted') +
        geom_point(data=subset(dfa,variable==levels(variable)[2]),size=1) +
            geom_point(data=subset(dfa,variable==levels(variable)[3] & value < 4),size=1) +
                geom_smooth(data=subset(dfa,variable==levels(variable)[3] & value <4)) +
                    geom_text(label="paste(sigma,'=',50)",parse=T,data=data.frame(n1=25,method="unadjusted",variable="sigma",value=1.25),size=18) +
...
For the unadjusted rule one may also suspect that the maximum absolute
variance bias is attained for $\sigma \rightarrow \Infty$. The results
of a simulation study where we set $\sigma=10$ (data not shown),
however, showed consistenly lower maximum values; therefore did not
support this theory. 
...
cplot <- ggplot(dfa) +
    geom_line(aes(n1,100*value,linetype=side,shape=side),data=subset(dfa,variable=="str_anc")) +
        geom_smooth(aes(n1,value),data=subset(dfa,variable!="str_anc")) +
            geom_point(aes(n1,value,shape=side),data=subset(dfa,variable!="str_anc"),size=1.5) +
                facet_grid(variable~method,scale='free_y',labeller=lverbose)+xlab(expression(n[1]))+ylab('')+theme_bw()+xlim(c(2,50))
...
library(bt88.03.704)
setwd('~/Dropbox/msi/2013/research/blind_confidence/rssversion/graphs/')
cairo_ps('blinded-max.eps',width=11,height=4.4,pointsize=8)
