...
The following table gives an index of which Figures in the original
paper correspond to which in this document. Note that Figure 4 in the
...
figure_table <- data.frame("Article Figure"=paste('Figure',c(1,2,3,4,5)),
                           "Supplemental Figure"=paste('Figure',c(3,4,2,"7, 13, 14",15)))
...
variance estimate ($n^u_2$). The dotted lines show the bias of the
mean estimate using a samplesize reassessment rule is used in order to
maximize the negative or positive bias, respectively.

```{r Bias_delta,results='hide',echo=F,warning=F,fig.cap="\\label{fig:bias}Bias of the mean under blinded sample size reassessment using the unadjusted (solid line) and adjusted (dashed line) interim variance estimate. Dotted lines show maximum negative and positive bias that can be attained under any blinded sample size reassessment rule according to Theorem 2 (red line); can be attained under any (unblinded) sample size reassessment rule (green line). The treatment effect used for planning is set to $\\delta_0=1$. Rows refer to the a priori assumed standard deviations $\\sigma_0$ determining the first stage sample size $n_1$. The columns correspond to actual standard deviations. The x-axis in each graph denotes the true treatment effect $\\delta$, the y-axis shows the bias."}
...
```{r Bias_sigma2,results='hide',echo=F,warning=F,fig.cap="\\label{fig:sd}Bias of the variance under blinded sample size reassessment using the unadjusted (solid line) and adjusted (dashed line) interim variance estimate. The red line gives the lower bound from Theorem 4 for the bias under sample size reassessment based on the unadjusted variance estimate. The treatment effect used for planning is set to $\\delta_0=1$. Rows refer to the a priori assumed standard deviations $\\sigma_0$ determining the first stage sample size $n_1$. The columns correspond to actual standard deviations. The x-axis in each graph denotes the true treatment effect $\\delta$, the y-axis shows the bias."}
...
negative bias. In the following figures we show $\hat S^2_e$,
$\sigma^2_{e}$ and $\sigma^2_{f}$ for trials using the adjusted
(Figure \ref{fig:varmeanBias}) and the adjusted sample size
reassessment rules. In we show the results for simulated trials using
the adjusted interim variance estimate (Figure
\ref{fig:uc.varmeanBias}).
...
1. We evaluate bias, and coverage probabilities using an initial
   ...
   and 50 over a relatively fine    grid of parameter values $\delta \in [0,4]$ and $\sigma \in [.5,4]$.
...
```{r  compare_maxvarbiases_2,echo=F,fig.cap="Comparison of maximal absolute variance bias for different parameter settings: Solid line - $\\delta=0$, $\\sigma=50$; Dashed line - $\\delta=0$, $\\sigma=4$; Dotted line $\\delta=0$, $\\sigma \\in [.5,4]$.",fig.align="center"}
...
cplot <- ggplot(dfa) +
    geom_line(aes(n1,-100*value+2.5),data=subset(dfa,variable=="str_anc"&side=="one-sided")) +
        geom_line(aes(n1,-100*value+5,linetype=1),data=subset(dfa,variable=="str_anc"&side=="two-sided")) +
...
Finally we perform a simulation based optimization of the inflation of
the non-coverage probability under the null hypotheses
(i.e. $\delta=0$). This corresponds to an inflation of the Type I
error rate of the associated test procedure.
