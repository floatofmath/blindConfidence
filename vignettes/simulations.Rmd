...
```{r load data maximum bias,eval=F,echo=F}
data(maxsim)
...
dfa <- df[df$scale=="absolute",]#"relative",]#
scalehelper <- data.frame(n1=rep(-10,3),variable=factor(1:3,label=c("maximum variance bias",expression(delta),expression(sigma))),value=rep(0,3),scale=rep("absolute",3),method=rep("adjusted",3),theoretical=3)
dfa <- rbind(dfa,scalehelper)

vplot <- ggplot(dfa,aes(n1,value))+
    geom_line(aes(n1,-value),subset=.(variable=="maximum variance bias")) + geom_line(aes(n1,-theoretical),subset=.(variable=="maximum variance bias" & method=='unadjusted'),linetype='dotted') + 
    ...
    facet_grid(variable~method,scale='free_y',labeller=lverbose(string="Absolute bias")) +xlab(expression(n[1]))+ylab('')+theme_bw()+xlim(c(2,50))
print(vplot)
...
G2coverage <- add_epsilon(maxcoverageopt1,.03,.01,c('delta','sigma'))
plot_grid(maxsim,maxcoverageopt1,G2coverage,'sigma')
options(mc.cores=min(20,detectCores()-1))
maxcoveragesim2 <- simulate_batch(G2coverage,5*10^6)
## reshape and resim
<<<<<<< HEAD
...
>>>>>>> b34daed67b4f1cbeb1be34315af55cc8f86a5b57
...
cplot <- ggplot(dfa)+
    geom_line(aes(n1,100*value,linetype=side,shape=side),subset=.(variable=="actual - nominal coverage"))  +
        geom_smooth(aes(n1,value),subset=.(variable!="actual - nominal coverage"))  +
            geom_point(aes(n1,value,shape=side),subset=.(variable!="actual - nominal coverage"),size=1.5)  +
                            facet_grid(variable~method,scale='free_y',labeller=lverbose(string='Actual - nominal [%]')) +xlab(expression(n[1]))+ylab('')+theme_bw()+xlim(c(2,50))
print(cplot)
