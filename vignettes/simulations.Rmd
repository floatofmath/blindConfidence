...
subtitle: "Supplementary Web Materials: Numerical results"
...
``Estimation after blinded interim analysis''. It is also provided as
...
interim variance estimate $n^a_2$. Negative values correspond to situations
...
### Coverage probability: adjusted interim variance estimate
...
### Coverage probability: unadjusted interim variance estimate


Next we show the difference between actual and nominal coverage
probabilities, for designs that use the unadjusted lumped variance at
interim for sample size reassessment (i.e. $n_2^u$).
...
groups. Solid lines show the bias in the mean estimate for designs where sample sizes
are adjusted based on the adjusted interim variance estimate ($n^a_2$), dashed
lines for designs where sample sizes are adjusted based on the
unadjusted interim variance estimate ($n^u_2$). The dotted lines show the
...
tupd.mean.bias <- ggplot(gridsim,mapping=aes(x=delta,y=mean.bias)) +
    ...
    geom_path(aes(y=m.bias),lty=3,col='red',data=subset(gridsim,m.bias<=0.2 & n1 == 2)) +
    geom_path(aes(y=m.bias.n),lty=3,col='red',data=subset(gridsim,m.bias.n>=-0.2 & n1 == 2)) +
    geom_path(aes(y=m.bias),lty=3,col='red',data=subset(gridsim,m.bias<=0.1 & n1 == 8)) +
    geom_path(aes(y=m.bias.n),lty=3,col='red',data=subset(gridsim,m.bias.n>=-0.1 & n1 == 8)) +
    geom_path(aes(y=m.bias),lty=3,col='red',data=subset(gridsim,m.bias<=0.1 & n1 == 18)) +
    geom_path(aes(y=m.bias.n),lty=3,col='red',data=subset(gridsim,m.bias.n>=-0.1 & n1 == 18)) +
    geom_path(aes(y=m.bias),lty=3,col='red',data=subset(gridsim,m.bias<=0.1 & n1 == 32)) +
    geom_path(aes(y=m.bias.n),lty=3,col='red',data=subset(gridsim,m.bias.n>=-0.1 & n1 == 32)) +

    geom_path(aes(y=uc.mean.bias),lty=1) +
    geom_path(aes(y=brannath),lty=3,col='darkgreen',data=subset(gridsim,brannath <= 0.1)) +
    geom_path(aes(y=-brannath),lty=3,col='darkgreen',data=subset(gridsim,brannath <= 0.1)) +
...
Next we show the bias when estimating the variance at the end of the
...
estimate of the true standard error of the mean. In the following
figures solid lines show the standard deviation of mean estimates
across simulated trials, dashed lines show the average standard error
estimates across simulated trials, dotted lines show the average of
the true standard errors of corresponding fixed sample designs. The
true standard error of the mean of an adaptive design with blinded
sample size reassessment is smaller than that of corresponding fixed
sample design. The bias of the estimate of the standard error is close
to zero around the null hypothesis.  This holds both for designs using
...
even though the variance estimate has a considerable negative
bias.


### Variance of the mean estimate: adjusted interim variance estimate

First we show the results for simulated trials using the adjusted
interim variance estimate ($n_2^a$).
...
### Variance of the mean estimate: unadjusted interim variance estimate

Second we show the results for simulated trials using the adjusted
interim variance estimate ($n_2^u$).
...
1. We evaluate bias, and coverage probabilities using an initial
   ...
   and 50 over a relatively fine    grid of parameter values $\delta \in [0,4]$ and $\sigma \in
   [.5,4]$.
...
In the following Figures we present the results from the final
simulation run in Step 3. For the bias of the variance we performed
some additional simulations as our results suggested that the
optimizing scenarios are given for $\delta=0$ and for the undadjusted
sample size rule also $\sigma=\infty$.
...
For each scenario we performed $10^6$ simulation runs.

Steps 2 and 3 of the maximizing procedure were then performed for each
quantity seperately.

## Maximum bias of the mean for given $n_1$
...
```{r plot labeller,results='hide',include=F,echo=F}
                                        #This little function takes care of annotating the plots.
lverbose  <- function(labels,from=c("anc","mmb","mvb"),to=c("non-coverage [%]","max. mean bias","abs. variance bias"),multi_line=TRUE){
...
previously simulations (points every .01 in a cubic vicinity -  with a
width of .1 - of the maximizing $\delta$ and $\sigma$)and simulate
again using $4*10^6$ runs. 
...
Finally we selected the maximal values of the bias and resimulate
using $10^7$ runs to remove any selection bias.
...
mplot <- ggplot(dfa)+
    ...
    geom_smooth(aes(n1,value),data=subset(dfa,variable!="str_mmb")) + geom_point(aes(n1,value),data=subset(dfa,variable!="str_mmb"),size=.2) +
...
vplot <- ggplot(dfa,aes(n1,value))+
    ...
    geom_smooth(data=subset(dfa,variable==levels(variable)[3])) + geom_point(data=subset(dfa,variable!="str_mvb"),size=.2) + geom_smooth(data=subset(dfa,variable==levels(variable)[2])) +
    facet_grid(variable~method,scale='free_y',labeller=lverbose) +xlab(expression(n[1]))+ylab('')+theme_bw()+xlim(c(2,50))
print(vplot)

```

It appears that the absolute variance bias (using both sample size
...
Especially, setting $\sigma=50$ increased the variance of the
estimated bias quite a bit so that the number of simulation runs had 
to be increased to $10^8$ per scenario to get sufficently smooth
results. The results of these simulations are shown and compared in
...
First we show that setting $\delta = 0$ does not decrease the absolute
variance bias compared to the maximum values found when $\delta$ was allowed
to range from $0$ to $4$.

```{r,fig.cap="Comparison of maximal absolute variance bias for different parameter settings: Solid line - $\\delta=0$, $\\sigma \\in [.5,4]$; Dashed line - $\\delta \\in [0,4]$, $\\sigma=[.5,4]$; transparent red line - theoretical bound."}
...
$\delta=0$ and $\sigma$ ranged from $.5$ to $4$. Additionally we show
that setting $\sigma=4$ does increase the bias compared to the
maximum values found when $\delta=0$ and $\sigma$ ranged from $.5$ to
$4$, but not as much as for $\sigma=50$. 



```{r compare_maxvarbiases_1,echo=F,fig.cap="Comparison of maximal absolute variance bias for different parameter settings: Solid line - $\\delta=0$, $\\sigma=50$; Dashed line - $\\delta=0$, $\\sigma=4$; Dotted line $\\delta=0, $\\sigma \\in [.5,4]$; transparent red line - theoretical bound."}
...
```{r,echo=F,fig.cap="Comparison of maximal absolute variance bias for different parameter settings: Solid line - $\\delta=0$, $\\sigma \\in [.5,4]$; Dashed line - $\\delta \\in [0,4]$, $\\sigma=[.5,4]$."}
...
Finally, we show that, for the adjusted sample size rule, setting
...
```{r  compare_maxvarbiases_2,echo=F,fig.cap="Comparison of maximal absolute variance bias for different parameter settings: Solid line - $\\delta=0$, $\\sigma=50$; Dashed line - $\\delta=0$, $\\sigma=4$; Dotted line $\\delta=0, $\\sigma \\in [.5,4]$."}
...
range of parameters that seem of practical concern. 
...
```{r maxVarbiasR,echo=F,fig.width=4,fig.height=5}
    ...
    geom_line(aes(n1,-value),data=subset(dfa,variable=="str_mvb")) + geom_line(aes(n1,-theoretical),data=subset(dfa,variable=="str_mvb" & method=='unadjusted'),size=1.5,alpha=.2) +
        geom_point(data=subset(dfa,variable==levels(variable)[2]),size=.2) +
            geom_point(data=subset(dfa,variable==levels(variable)[3] & value < 5),size=.2) +
                geom_smooth(data=subset(dfa,variable==levels(variable)[3] & value <4)) +
...
To improve our estimate of the maximum we refine the grid around
previously simulations (points every .005 in a cubic vicinity -  with a
width of .2 - of the maximizing $\delta$ and $\sigma$)and simulate
again using $4*10^6$ runs. 
...
```{r maxCoverage,echo=F,fig.width=5,fig.height=5}
...
Finally we reproduce the simulations shown in the case study of our
paper.
...
mplot <- ggplot(df) + facet_grid(variable~sigma,scales='free_y',labeller=lverbose) +
    ...
    geom_path(aes(delta,mean.upper.bias),lty=3,col='red',data=subset(df,variable  == "Bias of the mean" & mean.upper.bias < .8))+
    geom_path(aes(delta,-mean.upper.bias),lty=3,col='red',data=subset(df,variable  == "Bias of the mean" & mean.upper.bias < .8))+
    geom_path(aes(delta,brannath),lty=3,col='darkgreen',data=subset(df,variable  == "Bias of the mean" & brannath < 1.2))+
    geom_path(aes(delta,-brannath),lty=3,col='darkgreen',data=subset(df,variable  == "Bias of the mean" & brannath < 1.2))+
    ...
    geom_path(aes(delta,bound),col='red',data=subset(df,variable == "Bias of the variance")) +
    geom_path(aes(x=delta,y=100*(.025-uc.upper.prob)),lty=2,data=subset(df,variable=="Actual - nominal [%]" & abs(uc.upper.prob < .035))) +
    geom_line(aes(x=delta,y=0),colour='gray',data=subset(df,variable=="Actual - nominal [%]" & abs(uc.upper.prob < .035))) +
    geom_path(aes(x=delta,y=100*(.025-uc.lower.prob)),lty=3,data=subset(df,variable=="Actual - nominal [%]" & abs(uc.lower.prob < .035))) +
    geom_path(aes(x=delta,y=100*(.05-uc.total.prob)),data=subset(df,variable=="Actual - nominal [%]" & abs(uc.total.prob < .06))) +
    geom_blank(aes(x=-5,y=-0.7),data=subset(df,variable=="Actual - nominal [%]" & abs(uc.upper.prob < .035))) +
    geom_blank(aes(x=-5,y=0.05),data=subset(df,variable=="Actual - nominal [%]" & abs(uc.upper.prob < .035))) + ylab('')+xlab(expression(delta))+theme_bw()
