...
## reshape and resim
max.bias.resh <- rbindlist(list(within(max.bias[,1:4],{ variable  <-  "abs.mean"}),
                       ## within(max.bias[,c(1,5:7)],{ variable  <- "rel.mean"})
                       within(max.bias[,c(1,8:10)],{ variable  <- "uc.abs.mean"})),use.names=FALSE)
                       ## within(max.bias[,c(1,11:13)],{ variable  <- "uc.rel.mean"})),use.names=FALSE)
             
runs <- 10^2
system.time(resim <- mclapply(1:nrow(max.bias.resh),
                              function(i) {c(max.bias.resh[i,],
                                             simVBIA(delta=max.bias.resh[i,]$max.mean.bias.delta,
                                                     sigma=max.bias.resh[i,]$max.mean.bias.sigma,
                                                     d=1,
                                                     n1=max.bias.resh[i,]$n1,
                                                     cf=1,
                                                     runs=runs,
                                                     alpha=.025,
                                                     beta=.2))}))
...
resim[,relbias := mean.bias/(max.mean.bias.delta)]


max.bias2 <- ddply(resim,"n1",transform,
                  max.mean.bias= mean.bias,
                  ## max.mean.bias.vm = vm[which.min(mean.bias)],
                  max.rel.mean.bias= mean.bias/max.mean.bias.delta,
                  ## max.rel.mean.bias.vm = vm[which.min(relbias)],
                  uc.max.mean.bias= uc.mean.bias,
                  ## uc.max.mean.bias.vm = vm[which.min(uc.mean.bias)],
                   uc.max.rel.mean.bias= uc.mean.bias/max.mean.bias.delta,
                   method=c("abs.mean"="unadjusted","uc.abs.mean"="adjusted")[variable]
                  ## uc.max.rel.mean.bias.vm = vm[which.min(uc.relbias)]
                  ## max.variance.bias= -min(variance.bias),
                  ## max.variance.bias.delta = delta[which.min(variance.bias)],
                  ## max.variance.bias.sigma = sigma[which.min(variance.bias)],
...
max.bias2 <- max.bias2[,c('method','n1','max.mean.bias.delta','max.mean.bias.sigma','max.mean.bias','max.rel.mean.bias','uc.max.mean.bias','uc.max.rel.mean.bias')]
...
df <- df[df$variable %in% c('max.mean.bias','max.mean.bias.delta','max.mean.bias.sigma'),]
...
mplot <- ggplot(dfa)+
    geom_line(aes(n1,value),subset=.(variable=="maximum mean bias")) +
...
max.coverage.resh <- rbindlist(list(within(max.coverage[,1:4],{ variable  <-  "abs.variance"}),
                       within(max.coverage[,c(1,5:7)],{ variable  <- "uc.abs.variance"})),use.names=FALSE)

             
runs <- 10^2
system.time(resim <- mclapply(1:nrow(max.coverage.resh),
                              function(i) {c(max.coverage.resh[i,],
                                             simVBIA(delta=max.coverage.resh[i,]$max.variance.bias.delta,
                                                     sigma=max.coverage.resh[i,]$max.variance.bias.sigma,
                                                     d=1,
...
max.coverage2 <- max.coverage2[,c('method','n1','max.variance.bias.delta','max.variance.bias.sigma','max.variance.bias','max.rel.variance.bias','uc.max.variance.bias','uc.max.rel.variance.bias')]
...
cplot <- ggplot(dfa)+
    geom_line(aes(n1,100*value),subset=.(variable=="noncoverage")) +
    geom_smooth(aes(n1,value),subset=.(variable!="noncoverage")) + geom_point(aes(n1,value),subset=.(variable!="noncoverage"),size=1) +
