...
\title{Case study: estimation after blinded interim analyses}
...
tupd <- ggplot(tcasesim,mapping=aes(x=delta,y=.025-upper.prob)) +
    ...
    facet_grid(s~sigma,labeller=lverbose) + theme_bw() + ylim(-.02,.01) + xlab(expression(delta)) + ylab('AC-NC')
...
tupd.mean.bias <- ggplot(tcasesim,mapping=aes(x=delta,y=mean.bias)) + geom_path(lty=2)+
  ...
  geom_path(aes(y=uc.mean.bias),lty=1) + geom_path(aes(y=brannath),lty=3,col='darkgreen') + geom_path(aes(y=-brannath),lty=3,col='darkgreen') +
  facet_grid(s~sigma,labeller=lverbose) + theme_bw()  + xlab(expression(delta)) + ylab('Bias') + ggtitle('Mean Bias') +  ylim(-1.5,1.5)
...
multiplot(tupd.uc,tupd.mean.bias,tupd.variance.bias,
          titles=c(expression(paste("Actual - nominal coverage probabilities (",n[2]^u,")")),
              #expression(paste("Actual-nominal coverage probabilities (",n[2]^a,")")),
              "Mean Bias",
              "Variance Bias") #,
...
mplot <- ggplot(df) + facet_grid(variable~sigma,scales='free_y',labeller=lverbose) +
    ...
    geom_blank(aes(x=-5,y=0.01),subset=.(variable=="Actual - Nominal")) + ylab('')+xlab(expression(delta))
...
<<eval=F,echo=F>>=
## this chunk produces the file that goes into the paper
cairo_ps('case.eps',height=7,width=7)
...
<<oldplot,eval=F,echo=F,include=F>>
## this chunk produces the file that goes into the paper
