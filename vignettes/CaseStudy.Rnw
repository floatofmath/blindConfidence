...
The next chunk sets up the basic parameters taken from
\cite{kieser2003}. 
...
As an example \cite{kieser2003} present the trial reported in
\cite{malsch2001}, which is a randomized placebo controlled trial of
...
require(ggplot2)
gsim <- as.data.frame(t(lapply(casesim,unlist)))
...
max.mean.bias <- max(dsim$mean.bias)
uc.max.mean.bias <- max(dsim$uc.mean.bias)
min.mean.bias <- min(dsim$mean.bias)
uc.min.mean.bias <- min(dsim$uc.mean.bias)

max.var.bias <- min(dsim$variance.bias)
uc.max.var.bias <- min(dsim$uc.variance.bias)

#$
@ 

\cite{kieser2003} assume that sample size reassessment based on
...
$\Sexpr{round(uc.max.var.bias)}$, \ie it reaches the theoretical 
...
dsim <- subset(tdsim, sigma %in% (c(1:5)*2) )
tupd <- ggplot(dsim,mapping=aes(x=delta,y=.025-upper.prob)) +
    ...
    facet_grid(s~sigma,labeller=lverbose) + theme_bw() + ylim(-.02,.01) + xlab(expression(delta)) + ylab('actual - nominal coverage')
...
tupd.variance.bias <- ggplot(dsim,mapping=aes(x=delta,y=variance.bias)) +
  ...
  facet_grid(s~sigma,labeller=lverbose) + theme_bw() + xlab(expression(delta)) + ylab('bias')+ggtitle('Variance Bias')
...
tupd.ass <- ggplot(dsim,mapping=aes(x=delta,y=(mean.m1+n1))) +
  ...
  facet_grid(s~sigma,labeller=lverbose) + theme_bw() + xlab(expression(delta)) + ylab('average sample size')+ggtitle('Average Second Stage Sample Size')

pdf('casestudy.pdf')
multiplot(tupd.mean.bias,tupd.variance.bias,tupd.ass)
dev.off()
