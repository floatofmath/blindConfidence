...
runs <- 10^4
bsim <- do.call(rbind,mclapply(1:nrow(gsim),function(i) {c(gsim[i,],simMBIA(delta=gsim[i,]$delta,sigma=gsim[i,]$sigma,n1=gsim[i,]$n1,runs=runs))}))

bsim <- as.data.frame(apply(bsim,1,unlist),row.names=NA)
bsim <- as.data.frame(apply(bsim,1,unlist),row.names=NA)

gsim$mean.upper.bias <- bsim[,5]
gsim$mean.lower.bias <- bsim[,6]
