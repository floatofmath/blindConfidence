...
set.seed(50014056)
runs <- 10^6
gridsim <- t(simplify2array(mclapply(1:nrow(G),function(i) {c(G[i,],simVBIA(delta=G[i,]$delta,sigma=G[i,]$sigma,d=G[i,]$d,s=G[i,]$s,runs=runs,alpha=.025,beta=.2))})))
...
## recompute the first stage sample size that was used
...
dsim <- subset(gsim,sigma %in% c(.5,1,1.5,2) & d %in% c(1) & s %in% c(.5,1,1.5,2))


D <- subset(G,sigma %in% c(.5,1,1.5,2) & d %in% c(1) & s %in% c(.5,1,1.5,2))
dsim <- subset(gsim,sigma %in% c(.5,1,1.5,2) & d %in% c(1) & s %in% c(.5,1,1.5,2))
runs <- 10^5
bsim <- do.call(rbind,mclapply(1:nrow(D),function(i) {c(G[i,],bias.sim(delta=G[i,]$delta,sigma=G[i,]$sigma,n1=dsim[i,]$n1,runs=runs))}))
bsim <- as.data.frame(apply(bsim,1,unlist),row.names=NA)
bsim <- as.data.frame(apply(bsim,1,unlist),row.names=NA)

dsim$mean.upper.bias <- bsim[,9]
...
fname <- paste('/tmp/gridsim_',format(Sys.time(),"%y%m%d"),'.Rd',sep='')
...
lverbose = function(variable,value){
    if(variable == "s"){
        sapply(value,function(val) substitute(paste("s = ",foo,", ",n[1]," = ",n1,sep=''),
...
```
Next we look at estimates of the mean difference between treatment groups.

```{r}

tupd.mean.bias <- ggplot(gridsim,mapping=aes(x=delta,y=mean.bias)) + geom_path(lty=1)+
  geom_line(y=0,colour='gray') + geom_path(aes(y=mean.upper.bias),lty=3,col='red') +  geom_path(aes(y=uc.mean.bias),lty=2) +
...
As it turns out the estimate of the standard error is close to the
standard error of the estimated mean estimate across all simulation
...
```{r}
tupd.vm.bias <- ggplot(gridsim,mapping=aes(x=delta,y=vm)) +
    geom_line(y=0,colour='gray') + geom_path(aes(y=ev),colour='gray') + geom_path(aes(y=exv),colour='green') +
  geom_path(lty=1) + geom_path(aes(y=uc.vm),lty=2) +  geom_path(aes(y=uc.ev),colour='gray',lty=2) + geom_path(aes(y=uc.exv),colour='green',lty=2) +
  facet_grid(sigma~s,labeller=lverbose) + theme_bw() + xlab(expression(delta)) + ylab('bias')
print(tupd.vm.bias)

tupd.vm.diff <- ggplot(gridsim,mapping=aes(x=delta,y=-vm.diff)) +
    geom_line(y=0,colour='gray') + 
  geom_path(lty=1) + geom_path(aes(y=-uc.vm.diff),lty=2) +#geom_point(shape='V') + geom_point(aes(y=mean.bias),shape='M') +
  facet_grid(sigma~s,labeller=lverbose) + theme_bw() + xlab(expression(delta)) + ylab('bias')
print(tupd.vm.diff)
```
